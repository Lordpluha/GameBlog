...
export class TokenService {
    ...
    async generate(dto: JwtGenerateDto) {
        const accessToken = await this.jwtService.signAsync(dto, {expiresIn: "30m", secret: this.configService.get(SECRET_JWT_ACCESS)})
        const refreshToken = await this.jwtService.signAsync(dto, {expiresIn: "15d", secret: this.configService.get(SECRET_JWT_REFRESH)})
