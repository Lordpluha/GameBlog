...
export class CategoryService {
  ...
  async create(dto: CreateCategoryDto) {
    ...
    if (oldCategory)
      throw new BadRequestException(CATEGORY_WITH_NAME_ALREADY_EXISTS)
    const parent = {}
    ...
    await this.cacheManager.set(
      `category-${category.id}-no-parent`,
      category,
      this.CACHE_TIME
    )
    return category
  ...
  async update(id: number, dto: UpdateCategoryDto) {
    ...
    const dataUp = dto.name
      ? { slug: generateSlug(dto.name + id), name: dto.name }
      : {}
    const parent = {}
    if (dto.parentId) {
      parent['connect'] = { id: dto.parentId }
      ...
      if (!parentCategory)
        throw new NotFoundException(CATEGORY_WITH_ID_PARENT_NOT_FOUND)
